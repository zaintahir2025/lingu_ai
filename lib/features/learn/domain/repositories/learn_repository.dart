...
class LearnRepository {
  ...
  Future<void> completeLesson(int lessonId) async {
    await (_db.update(_db.lessons)..where((t) => t.id.equals(lessonId))).write(
      const LessonsCompanion(isCompleted: Value(true)),
    );

    await (_db.update(_db.lessons)..where((t) => t.id.equals(lessonId + 1)))
  ...
  Future<List<VocabWord>> getVocabForLesson(
    int lessonId, {
    String? targetLang,
    String? uiLocale,
  }) async {
    final defaultWords = await (_db.select(
      _db.vocabWords,
    )..where((t) => t.lessonId.equals(lessonId))).get();
    final langCode =
        targetLang ?? _onboardingStorage?.targetLanguage?.toLowerCase() ?? 'es';
